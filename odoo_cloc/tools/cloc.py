...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        _logger.info('MANIFEST: %s' % str(openerp.modules.module.MANIFEST))
        for i in openerp.modules.module.MANIFEST:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [openerp.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        _logger.info('Lista de modulos: %s' % module_list)
            ...
            if 1 == 1:
                ...
                if module_path:
                    if any(module_path.startswith(i) for i in exclude_path):
                        _logger.info('modulo excluido por el path: %s\n' % exclude_path)
                        continue
    ...
    def count_database(self, database):
        registry = openerp.registry(database if database else config['db_name'])
